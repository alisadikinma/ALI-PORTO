...
<body>
    ...
    <script>
        // Simple and effective session monitoring
        function checkAuthAndRedirect() {
            // Check if we're on an admin page but not authenticated
            const isAdminPage = window.location.pathname.includes('/galeri') || 
                               window.location.pathname.includes('/dashboard') ||
                               window.location.pathname.includes('/berita') ||
                               window.location.pathname.includes('/layanan') ||
                               window.location.pathname.includes('/project') ||
                               window.location.pathname.includes('/testimonial') ||
                               window.location.pathname.includes('/award') ||
                               window.location.pathname.includes('/contacts') ||
                               window.location.pathname.includes('/setting');
            
            const hasAuthElements = document.querySelector('.navbar-nav .dropdown-toggle') !== null;
            
            // If on admin page but no auth elements, redirect immediately
            if (isAdminPage && !hasAuthElements) {
                console.log('Not authenticated on admin page, redirecting...');
                window.location.replace('/login');
                return;
            }
            
            // Additional AJAX check for authenticated pages
            if (isAdminPage && hasAuthElements) {
        ...
        setInterval(checkAuthAndRedirect, 2 * 60 * 1000);
...
</html><?php /**PATH C:\xampp\htdocs\ALI-PORTO\resources\views/layouts/index.blade.php ENDPATH**/ ?>