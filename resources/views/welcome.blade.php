...
<section id="about" class="w-full max-w-screen-2xl mx-auto px-6 py-12">
    <div class="flex flex-col lg:flex-row justify-between items-center gap-12">
        ...
        <div class="flex-1 flex items-stretch justify-center order-1 lg:order-2">
            <div class="w-full max-w-lg lg:max-w-xl xl:max-w-2xl p-6 bg-slate-800 rounded-2xl outline outline-2 outline-orange-400 min-h-[400px] lg:min-h-[500px] xl:min-h-[600px]">
...
<section id="awards" class="w-full max-w-screen-2xl mx-auto px-4 sm:px-6 py-12">
    ...
    <div class="max-w-6xl mx-auto">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
            ...
            <div class="p-6 {{ $cardBg }} rounded-2xl border border-slate-600 hover:border-yellow-400/50 transition-all duration-300 hover:transform hover:scale-105">
                <!-- Award Icon & Title -->
        ...
        <div class="flex justify-center">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-4xl">
                ...
                <div class="p-6 bg-slate-800/80 rounded-2xl border border-slate-600 hover:border-yellow-400/50 transition-all duration-300 hover:transform hover:scale-105">
    ...
    <div class="flex flex-col lg:flex-row gap-8 lg:gap-12">
        ...
        <div class="lg:w-1/2 space-y-4">
            ...
            <div class="service-card {{ $index == 0 ? 'active' : '' }}" 
                 ...
                 data-description="{!! addslashes($row->keterangan_layanan) !!}">
                ...
                <div class="service-content">
                    ...
                    <p class="service-subtitle">{!! Str::limit(strip_tags($row->keterangan_layanan), 80) !!}</p>
                </div>
            </div>
            @endforeach
        </div>
        ...
        <div class="lg:w-1/2">
            <div class="service-display">
                ...
                <div class="service-description">
                    <p id="currentServiceDescription" class="description-text">
                        {!! $layanan[0]->keterangan_layanan ?? 'I provide tailored AI solutions and custom GPT models designed to meet your business needs and industry requirements. From understanding your challenges to delivering a solution, I make sure the AI tools we create truly support your goals and make your processes smarter.' !!}
                    </p>
                    
                    <div class="service-action">
                        <a href="{{ $konf->primary_button_link ?? '#contact' }}" class="request-quote-btn-services">
                            Request Quote →
                        </a>
                    </div>
                </div>
    ...
    serviceCards.forEach(card => {
        card.addEventListener('click', function() {
            ...
            if (newDescription) {
                ...
                setTimeout(() => {
                    serviceDescription.innerHTML = newDescription;
    ...
    <div class="w-full max-w-full sm:max-w-5xl grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8" id="portfolio-grid">
        ...
        <div class="portfolio-item p-6 sm:p-8 bg-slate-800/60 rounded-2xl outline outline-1 {{ $loop->iteration == 3 ? 'outline-yellow-400' : 'outline-neutral-900/40' }} backdrop-blur-xl flex flex-col gap-6 transition-opacity duration-300 ease-in-out" data-jenis="{{ $project->jenis_project }}">
            <a href="{{ route('portfolio.detail', $project->slug_project) }}">
                <img class="w-full max-w-[300px] sm:max-w-[400px] h-auto rounded-2xl aspect-[5/4] object-cover" src="{{ asset('file/project/' . $project->gambar_project) }}" alt="{{ $project->nama_project }}" />
            </a>
...
<section id="gallery" class="w-full max-w-screen-xl mx-auto px-3 sm:px-4 py-6 flex flex-col items-center gap-6 sm:gap-10">
    ...
    <div id="galleryGrid" class="grid grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6 w-full">
        ...
        <div>
            <div class="relative group rounded-lg bg-slate-900 outline outline-1 outline-slate-500 overflow-hidden transition-transform duration-300 hover:scale-105 hover:shadow-md">
                <img src="{{ asset('file/galeri/' . $row->gambar_galeri) }}" alt="{{ $row->nama_galeri ?? 'Gallery image' }}" class="w-full h-auto rounded-lg aspect-square object-cover cursor-pointer gallery-image" />
                <div class="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-40 transition-opacity duration-300 flex items-end pointer-events-none">
                    <p class="w-full text-center text-white text-xs sm:text-sm font-semibold p-1 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
...
<section id="contact" class="w-full max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8 py-10 sm:py-14 bg-slate-800 rounded-3xl border border-slate-700 -m-1 flex flex-col lg:flex-row gap-8 lg:gap-12">
    ...
    <form action="{{ route('contact.store') }}" method="POST" class="flex flex-col gap-6 sm:gap-8 flex-1">
        ...
        <div class="flex flex-col gap-4">
            ...
            <div class="flex flex-col sm:flex-row gap-4">
                <select name="service" class="w-full sm:w-1/2 h-12 bg-slate-800 rounded-md border border-slate-600 px-4 text-white focus:outline-none focus:ring-2 focus:ring-yellow-400">
                    <option value="">Select Service</option>
                    <option value="ai">AI Development</option>
                    <option value="automation">Automation</option>
                </select>
                <select name="budget" class="w-full sm:w-1/2 h-12 bg-slate-800 rounded-md border border-slate-600 px-4 text-white focus:outline-none focus:ring-2 focus:ring-yellow-400">
