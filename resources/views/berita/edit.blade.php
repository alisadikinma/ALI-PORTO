    ...
    <div class="col-lg-8">
        <div class="card">
            ...
            <div class="card-body">
                ...
                <form action="{{ route('berita.update', $berita->id_berita) }}" method="POST" enctype="multipart/form-data" id="articleForm">
                    @csrf
                    @method('PUT')
                    ...
                    <!-- Featured Image -->
                    <div class="form-group mb-4">
                        <label for="gambar_berita" class="form-label fw-semibold">Featured Image</label>
                        @if($berita->gambar_berita)
                        <div class="mb-2">
                            <img src="{{ asset('file/berita/' . $berita->gambar_berita) }}" class="img-thumbnail" width="200">
                    ...
                    <div class="form-group mb-4">
                        ...
                        <textarea name="featured_snippet" 
                                  ...
                                  maxlength="300">{{ old('featured_snippet', $berita->featured_snippet) }}</textarea>
                        <div class="form-text">
                            <small class="text-muted">
                    ...
                    <div class="form-group mb-4">
                        ...
                        <textarea name="conclusion" 
                                  ...
                                  maxlength="500">{{ old('conclusion', $berita->conclusion) }}</textarea>
                        <div class="form-text">
                            <small class="text-muted">
        ...
        <div class="card mt-4">
            ...
            <div class="card-body">
                <div class="position-relative">
                    ...
                    <input type="hidden" name="tags" id="tagsHidden" form="articleForm" value="{{ $berita->tags }}">
                </div>
        ...
        <div class="card mt-4">
            ...
            <div class="card-body">
                <div id="faq-container">
                    @php
                        $faqData = is_string($berita->faq_data) ? json_decode($berita->faq_data, true) : $berita->faq_data;
                    @endphp
                    @if($faqData && count($faqData) > 0)
                        ...
                        <div class="faq-item mb-3">
                            ...
                            <textarea name="faq_answers[]" 
                                      ...
                                      form="articleForm">{{ $faq['answer'] ?? '' }}</textarea>
                        </div>
                        @endforeach
                    @else
                        <div class="faq-item mb-3">
            ...
            <div class="card-body">
                <div class="form-group mb-3">
                    ...
                    <select name="kategori_berita" id="kategori_berita" class="form-select" required>
                        ...
                        <option value="AI & Tech" {{ $berita->kategori_berita == 'AI & Tech' ? 'selected' : '' }}>AI & Technology</option>
                        <option value="Web Development" {{ $berita->kategori_berita == 'Web Development' ? 'selected' : '' }}>Web Development</option>
                        <option value="Tutorial" {{ $berita->kategori_berita == 'Tutorial' ? 'selected' : '' }}>Tutorial</option>
                        <option value="Industry News" {{ $berita->kategori_berita == 'Industry News' ? 'selected' : '' }}>Industry News</option>
                    </select>
                </div>

                <div class="form-group mb-3">
                    <div class="form-check">
                        <input type="checkbox" name="is_featured" id="is_featured" class="form-check-input" value="1" {{ $berita->is_featured ? 'checked' : '' }}>
                ...
                <div class="form-group mb-3">
                    ...
                    <input type="datetime-local" 
                           ...
                           id="tanggal_berita"
                           class="form-control" 
                           value="{{ $berita->tanggal_berita ? $berita->tanggal_berita->format('Y-m-d\TH:i') : '' }}">
                </div>
            </div>
        </div>
            ...
            <div class="card-body">
                ...
                <div class="form-group mb-3">
                    ...
                    <input type="text" 
                           ...
                           value="{{ old('focus_keyword', $berita->focus_keyword) }}">
...
let selectedArticles = {!! json_encode($berita->relatedBerita()->map(function($item) {
    return ['id' => $item->id_berita, 'title' => $item->judul_berita, 'category' => $item->kategori_berita];
})->toArray()) !!};

let tags = {!! json_encode($berita->tags ? array_map('trim', explode(',', $berita->tags)) : []) !!};
...
// Display existing related articles
...
function searchArticles(query) {
    // Get current article ID to exclude
    const currentId = {{ $berita->id_berita }};
    const excludeIds = [currentId, ...selectedArticles.map(a => a.id)];
    
    // AJAX call to search articles
    fetch(`/api/articles/search?q=${encodeURIComponent(query)}&exclude[]=${excludeIds.join('&exclude[]=')}`)
...
let faqCount = {{ $faqData && count($faqData) > 0 ? count($faqData) : 1 }};
function addFAQ() {
    faqCount++;
